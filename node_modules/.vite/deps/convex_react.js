...
var includePage = /* @__PURE__ */ Symbol("includePageKeys");
var page = /* @__PURE__ */ Symbol("page");
...
function usePaginatedQuery_experimental(query, args, options) {
  ...
  const [state, setState] = (0, import_react2.useState)(createInitialState);
  let currState = state;
  if (getFunctionName(query) !== getFunctionName(state.query) || JSON.stringify(convexToJson(argsObject)) !== JSON.stringify(convexToJson(state.args)) || skip !== state.skip) {
    currState = createInitialState();
    setState(currState);
...
function usePreloadedQuery(preloadedQuery) {
  const args = (0, import_react4.useMemo)(
    () => jsonToConvex(preloadedQuery._argsJSON),
    [preloadedQuery._argsJSON]
  );
  const preloadedResult = (0, import_react4.useMemo)(
